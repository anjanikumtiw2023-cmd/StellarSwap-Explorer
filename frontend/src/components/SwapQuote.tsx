...
export function SwapQuote({ quote, outputCode }: { quote: QuoteResult | null; outputCode: string }) {
  ...
  return <section className="swap-quote" aria-live="polite" aria-label="Read-only swap estimate">
    ...
    <div><span>Best displayed price</span><strong>{quote.bestPrice}</strong></div>
    <div><span>Price impact</span><strong>{Number(quote.priceImpactBps) / 100}%</strong></div>
    ...
    <small>Estimate only, based on the current visible orderbook. It is not a guaranteed execution price.</small>
  </section>
}
